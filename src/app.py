...
def predict(query, category):
    ...
    if not results:
        ...
        for idx, rec in enumerate(results):
            # We use Gradio CSS variables so it looks perfect in both Light and Dark mode
            output_html += f"""
...
.gradio-container {
    /* Optional: add a subtle gradient reflection at the top of the app */
    background: radial-gradient(circle at 50% 0%, rgba(59, 130, 246, 0.08) 0%, transparent 50%);
}
"""

with gr.Blocks(title="CompliQ - BIS Standards Finder", css=css) as demo:
    with gr.Column(elem_id="header"):
        gr.Markdown(
            ...
            <div style="text-align: center; max-width: 800px; margin: 0 auto; padding: 20px 0;">
                ...
                <p style="font-size: 1.1rem; color: var(--body-text-color);">Describe your building material below to find the applicable <strong>Bureau of Indian Standards (BIS)</strong> regulations using our high-speed, hallucination-free AI pipeline.</p>
            </div>
            """
        )
    ...
    with gr.Row():
        with gr.Column(scale=3):
            query_input = gr.Textbox(
                lines=4, 
                placeholder="E.g., I manufacture high-strength deformed steel bars for concrete reinforcement. What are the standards?",
    ...
    # Using HTML for premium formatting instead of basic Markdown
    ...
    submit_btn.click(
        fn=predict,
        inputs=[query_input, category_dropdown],
        outputs=[output_html, latency_text]
    )
